...
aws s3api wait object-exists --bucket ${projectName}bucketcodedeploy --key app.zip
aws s3api wait object-exists --bucket ${projectName}bucketcodedeploy --key app.zip
aws cloudformation deploy --template ebs.yml --stack-name ${projectName}ebs --parameter-overrides ProjectName=${projectName} --capabilities CAPABILITY_IAM CAPABILITY_NAMED_IAM
